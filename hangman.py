...
class Hangman:
    ...
    def check_guess(self, guess):
        ...
        if lengthOfGuess < 1:
            raise ResponseLengthError(
                "Guess must be at least one character in length. Please try again."
            )

        for letter in range(0, lengthOfGuess):

            if ord(guess[letter].upper()) < ord("A") or ord(
                guess[letter].upper()
            ) > ord("Z"):
                raise InvalidInputHangmanError(
                    "Invalid input. Must be a character or word with no special characters. Please try again."
                )
    ...
    def get_game_state(self):
        ...
        state = (
            f"You have {self.__strikes} strikes.\nWord To Guess: {self.__str__()}\n"
